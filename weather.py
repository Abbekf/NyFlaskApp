...
# Funktion för att hämta väderdata för en stad och returnera parametrar som ska visas i webbläsaren
def get_weather(city): # Funktion för att hämta väderdata för en stad 
    params = { # Parametrar som skickas med i API-anropet om vad vi vill ha för data
    ...
    if response.status_code == 200:  # Om anropet lyckades så returneras väderdatan
        data = response.json() # omvandlar svaret från API:et till dictionary
